...
class LatestCollection extends StatefulWidget {
  const LatestCollection({Key? key}) : super(key: key);

  @override
  State<LatestCollection> createState() => _LatestCollectionState();
}

class _LatestCollectionState extends State<LatestCollection> {
  List<Hits> wallpapers = [];
  var data;
  Future<List<Hits>> getCategoriesWallpaper() async {
    http.Response response = await http.get(Uri.parse(
        "https://pixabay.com/api/?key=30448284-931053eb23d75c729b26b4d5a&image_type=all&per_page=200&order=latest&pretty=true"));
